...
//	finished.s
//
//	This program was created by using our 'newapp64' development
//	tool and then modifying the text of the message displayed by
//	its 'prog64' procedure and also the 64-bit memory-address at 
//	which that message is written (presumably to the 'relocated'
//	video display memory).  But this program crashes because its 
//	author neglected to 'map' the video memory page-frame, which
//	resides at physical-address 0xB8000, to the page-frame being
//	referenced in this program's virtual address-space (which is
//	located at the canonical 64-bit address 0x0000008040201000).
//
//				PROBLEM
//
//	Your job is to add the missing page-mapping tables and their
//	entries so that this program correctly displays its message,
//	without making any changes to the program's '.text' section.
//
//	 to assemble: $ as finished.s -o finished.o 
//	 and to link: $ ld finished.o -T ldscript -o finished.b 
//	 and install: $ dd if=finished.b of=/dev/sda4 seek=1 
//
...
#-------------------------------------------------------------------
msg1:	.ascii	" Now finishing the CS 686 Final Exam " 
len1:	.quad	. - msg1 
att1:	.byte	0x50     
#-------------------------------------------------------------------
prog64:	.code64 
