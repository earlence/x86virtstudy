...
//	correction: 13 JUL 2007 -- fixed an address scale-factor 
//-----------------------------------------------------------------
...
host_task:
	...
	mov	$0x480, %ecx
nxmsr:	rdmsr
	mov	%eax, msr0x480+0(, %rbx, 8)	# fixed 7/13/2007	
	mov	%edx, msr0x480+4(, %rbx, 8)	# fixed 7/13/2007
	inc	%ecx
	inc	%rbx
	cmp	$11, %rbx
	jb	nxmsr
...
nxnyb:	rol	$4, %rax 		# next nybble into AL 
	...
	ret 
#-------------------------------------------------------------------
# NOTE: Thanks to Martin Mocko for spotting the scale-factor error.
#-------------------------------------------------------------------
